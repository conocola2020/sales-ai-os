...
import { findInstagramUrl, extractUsername } from '../src/lib/instagram-detector'
...
// ─── website の種別判定 ────────────────────────
// Google Places の websiteUri は自社HPとは限らない。
// SNS・グルメポータルを自社HPとして巡回すると、ログインフォーム等を
// 問い合わせフォームと誤検出するため、巡回前にドメインで振り分ける。

const SNS_HOSTS = ['twitter.com', 'x.com', 'facebook.com', 'line.me', 'lin.ee', 'tiktok.com', 'youtube.com', 'youtu.be']
const PORTAL_HOSTS = ['tabelog.com', 'hotpepper.jp', 'retty.me', 'gnavi.co.jp', 'ubereats.com', 'demae-can.com']
const IG_NON_USERNAME_PATHS = ['p', 'reel', 'explore', 'stories', 'accounts']
...
function classifyWebsite(website: string): WebsiteKind {
  let hostname: string
  try {
