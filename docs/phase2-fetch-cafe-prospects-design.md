...
| 項目 | 値 |
| --- | --- |
| 取得対象 | カテゴリ `cafe` + `coffee_shop` |
| 検索エリア | 愛知県のみ（名古屋市16区＋主要数市）。**他県への拡張は行わない** |
| 目標件数 | **1,000件**（採用対象 status='new'、Enterprise SKU 無料枠内・完全無料想定） |
...
├── search-areas.ts                   # 愛知県エリア定義（拡張先含む）
└── types.ts                          # 共通型定義
```
...
export interface CafeProspectRow {
  ...
  status: 'new' | 'excluded'            // ブラックリストヒットは 'excluded'
  excluded_reason: string | null        // 'blacklist:starbucks' 等
...
export interface FetchStats {
  ...
  savedCount: number        // status='new' で INSERT 成功
  excludedSavedCount: number // status='excluded' で INSERT 成功
  errorCount: number
}
```
...
### 5. `src/lib/cafe-prospects/search-areas.ts`
...
```ts
// グループ別の採用カウンタを保持
const groupCounts = new Map<string, number>()  // group ID → 採用件数
...
for (const area of areas) {
  ...
  // ... 検索実行 ...
  // 採用件数を加算
  groupCounts.set(groupKey, already + adoptedInThisQuery)

  // --target 達成で全体終了
  if (totalAdopted >= target) {
    log(`🎯 目標 ${target}件に到達。処理終了。`)
    break
  }
}
```
...
[6] Supabase upsert
    100件単位でバッチ upsert（onConflict='place_id'）
        ↓
[7] 目標到達チェック
    採用件数（status='new'）が --target（既定1000）に達したら処理終了
    達していなければ次エリアへ。**他県への拡張は行わない**
        ↓
[8] サマリー出力
```
...
| 愛知 Vol.1（50クエリ） | 取得約2,500〜3,000件 → 重複排除後 1,500〜2,000件 → ブラックリスト除外後 採用 **1,000件で打ち切り** |
| ブラックリスト除外率 | 全体の 5〜15% 程度（保持） |
...
- **Enterprise SKU 無料枠 1,000件/月の範囲内、課金 $0 想定**
- **他県への拡張は行わない**
- 完了まで推定 20〜30分
...
✅ 完了
   ...
   取得（重複排除前）: 6,243件
   重複排除後: 2,981件
   ブラックリスト除外: 387件（status='excluded' で保存）
   採用（status='new'）: 2,594件
   保存成功: 2,594件
   保存失敗: 0件
   所要時間: 42分18秒
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async function callPlacesApi(...): Promise<...> {
  ...
  if (apiRequestCount > MAX_API_REQUESTS) {
    ...
    console.warn('   想定: 約125件 / Enterprise SKU 無料枠: 1,000件/月')
...
3. `src/lib/cafe-prospects/search-areas.ts` を作成
4. `src/lib/cafe-prospects/places-client.ts` を作成（先に `--dry-run` で動作確認）
5. `scripts/fetch-cafe-prospects.ts` 本体を作成
6. `package.json` に `fetch:cafe` script 追加
7. `.env.local` に `GOOGLE_PLACES_API_KEY`, `OWNER_USER_ID` を追加（ユーザーに案内）
8. **Step 3-1**: `npm run fetch:cafe -- --area nagoya-naka --limit 50` で本番テスト
9. 結果を Supabase Table Editor で目視確認
10. 問題なければ **Step 3-2**: 全域実行
...
- 50クエリ × 平均2.5ページ ≒ **約 125 リクエスト**
- Enterprise SKU 無料枠: **1,000 リクエスト/月**
- → Vol.1 完了時点で無料枠の約 **12.5%** を消費
- → **想定課金額: $0**

### 無料枠を超えた場合の単価（参考、Vol.2 以降の検討材料）

- Enterprise SKU: $0.035/req（最初の100,000req後は段階的割引）
- 仮に Vol.1 が想定の倍（250リクエスト）になっても無料枠内で完結
- `MAX_API_REQUESTS = 200` のセーフガードで実質的に課金リスクをゼロ化
