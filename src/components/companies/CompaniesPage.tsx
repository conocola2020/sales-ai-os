    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 md:px-6 py-3 md:py-4 border-b border-gray-200 flex-shrink-0">
        <div className="min-w-0 flex-1">
          <h1 className="text-base md:text-lg font-semibold text-gray-900">企業分析AI</h1>
          <p className="text-[10px] md:text-xs text-gray-500 mt-0.5 truncate">
            URLを入力するだけで企業情報・課題・提案ポイントをAIが自動分析します
          </p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-violet-500/10 border border-violet-500/20 rounded-lg text-xs font-medium text-violet-600">
          ...
          <button
            ...
            className="md:hidden flex items-center gap-1.5 px-2.5 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-400 hover:text-gray-900 transition-colors"
          >
            <Search className="w-3 h-3" />
            履歴
            {analyses.length > 0 && (
              <span className="px-1 py-0.5 bg-violet-500/20 text-violet-600 rounded text-[10px]">{analyses.length}</span>
            )}
          </button>
        </div>
      </div>
      ...
      {showHistory && (
        <div className="md:hidden border-b border-gray-200 p-4 max-h-64 overflow-y-auto bg-white/50">
          <AnalysisHistory
            analyses={analyses}
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-[1fr_300px] gap-0 overflow-hidden">
        {/* Left: Analysis area */}
        <div className="flex flex-col overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-5 md:border-r border-gray-200">
          {/* URL Input */}
          <div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-all disabled:opacity-60"
                />
              </div>
              <button
              ...
              {savedLeadId ? (
                <div className="flex items-center justify-between p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 rounded-lg text-xs font-semibold text-emerald-600 transition-colors whitespace-nowrap"
                  >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={handleGoToCompose}
                    className="flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-200 border border-gray-200 text-gray-700 text-xs font-medium rounded-xl transition-colors"
                  >
