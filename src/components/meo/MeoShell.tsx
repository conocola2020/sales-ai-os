    ...
    <aside className="w-72 md:w-64 h-full bg-gray-950 border-r border-gray-800/50 flex flex-col">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.label}
              {item.badge && (
                <span className="ml-auto px-1.5 py-0.5 rounded-md bg-emerald-500/15 text-emerald-400 text-[10px] font-bold">
                  {item.badge}
                </span>
              )}
            </Link>
      ...
      <div className="px-3 py-4 border-t border-gray-800/50 space-y-1">
        <Link
          href="/dashboard"
          onClick={onClose}
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-500 hover:text-white hover:bg-gray-800/60 transition-colors"
        >
        ...
        <div className="flex items-center justify-between px-3 py-2">
          ...
          <button
            ...
            className="w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-lg text-gray-500 hover:text-white hover:bg-gray-800 transition-colors"
            aria-label="ログアウト"
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="md:hidden flex items-center gap-3 px-4 py-3 bg-gray-950 border-b border-gray-800/50 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-white">{MEO_PRODUCT_NAME}</span>
          </div>
        </header>
