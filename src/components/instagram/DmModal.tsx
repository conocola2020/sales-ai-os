      ...
      <div className="relative z-10 w-full max-w-lg bg-white border border-gray-200 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900">@{target.username}</p>
              {target.display_name && (
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          {safetyStatus && safetyStatus.canSendNow && safetyStatus.waitSeconds > 0 && !target.dm_sent && (
            <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 rounded-lg px-4 py-3 text-sm text-amber-600">
              <AlertCircle className="w-4 h-4 shrink-0" />
          ...
          {error && (
            <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-3 text-sm text-red-600">
              <AlertCircle className="w-4 h-4 shrink-0" />
              {error}
              <button onClick={() => setError(null)} className="ml-auto text-red-600/60 hover:text-red-600">✕</button>
            </div>
          )}

          {/* Target info summary */}
          {(target.bio || target.industry || target.follower_count != null) && (
            <div className="bg-gray-50/50 border border-gray-200 rounded-xl p-3 space-y-1.5">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-500 focus:outline-none focus:border-violet-500 transition-colors resize-none"
            />
          </div>
          ...
          <div className="flex gap-2">
            ...
            {dmContent.trim() && (
              <button
                ...
                disabled={generating}
                title="再生成"
                className="p-2.5 bg-gray-50 hover:bg-gray-200 text-gray-400 hover:text-gray-900 rounded-xl border border-gray-200 transition-all disabled:opacity-50"
              >
          ...
          {dmContent.trim() && (
            <div className="flex gap-2">
              <button
                onClick={handleCopy}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-50 hover:bg-gray-200 border border-gray-200 hover:border-gray-300 text-gray-700 hover:text-gray-900 rounded-xl text-sm font-medium transition-all"
              >
                {copied ? (
                  <>
          ...
          {target.dm_sent && !target.dm_replied && (
            <div className="border border-amber-500/20 bg-amber-500/5 rounded-xl p-4">
              <p className="text-xs font-medium text-amber-600 mb-3">💌 返信を受け取りましたか？</p>
              <textarea
                rows={2}
                value={replyNote}
                onChange={e => setReplyNote(e.target.value)}
                placeholder="返信内容のメモ（任意）"
                className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 placeholder-gray-500 focus:outline-none focus:border-amber-500 transition-colors resize-none mb-2"
              />
              <button
                onClick={handleMarkReplied}
                disabled={savingReply}
                className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/30 text-amber-600 rounded-lg text-sm font-medium transition-all disabled:opacity-50"
              >
          ...
          {target.dm_replied && (
            <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-4 py-3 text-sm text-emerald-600">
        ...
        <div className="px-6 py-4 border-t border-gray-200 shrink-0 flex items-center gap-3">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm text-gray-400 hover:text-gray-900 hover:bg-gray-100 transition-colors">
            閉じる
          </button>
          ...
          {target.dm_sent && dmContent !== target.dm_content && (
            <button
              ...
              disabled={saving}
              className="ml-auto px-4 py-2 rounded-xl text-sm font-medium bg-gray-200 hover:bg-gray-300 text-gray-900 disabled:opacity-50 transition-colors"
            >
              {saving ? '保存中...' : '内容を更新する'}
            </button>
          )}
