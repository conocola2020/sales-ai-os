...
import { getDealStats, getUpcomingDeals } from './deals/actions'
import { getTargetStats } from './instagram/actions'

export default async function DashboardPage() {
  const [leadSummary, sendResult, replyResult, dealResult, igResult, upcomingResult] = await Promise.all([
    getLeadSummary(),
    getSendStats(),
    getReplyStats(),
    getDealStats(),
  ...
  return (
    <DashboardOverview
      ...
      sendStats={sendResult.data ?? { total: 0, reviewing: 0, sent: 0, failed: 0, manual: 0, formNotFound: 0, unsendable: 0 }}
      ...
      upcomingDeals={upcomingResult.data}
    />
  )
}
