# Sales AI OS - Codex Instructions
...
2. **Codex in Chrome MCP** (`mcp__Claude_in_Chrome__*`) — 「送信して」と指示された場合に `.Codex/skills/bulk-form-send.md` スキルに従って実行。ブラウザ経由なので403にならない。
...
2. **スキルファイル** `.Codex/skills/bulk-form-send.md` の手順に従う
...
- **スキルファイル**: `.Codex/skills/bulk-form-send.md`
...
## Codex in Chromeへの指示ルール

Codex in Chromeにフォーム送信を依頼する際は、
必ず以下の文言を指示の冒頭に付けること：

---
【送信前に必ず確認】
以下に1つでも該当したら送信を中止してSlackに通知すること：
- ページタイトルに「予約/宿泊/booking/reserve」が含まれる
- URLに「booking/reserve/yoyaku」が含まれる
- フォームフィールドに「チェックイン/泊数/人数/金額」が含まれる
- 送信ボタンに「予約する/申し込む/book now」が含まれる
---
